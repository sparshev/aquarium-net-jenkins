...
package ci.aquarium.net;

import ci.aquarium.net.config.AquariumCloudConfiguration;
import ci.aquarium.net.config.AquariumLabelTemplate;
...
public class AquariumCloud extends Cloud {
    ...
    private transient Map<String, ci.aquarium.net.model.Label> fishLabelsCache = new ConcurrentHashMap<>();
    ...
    private void setupLabelListeners() {
        client.addLabelChangeListener(new AquariumClient.LabelChangeListener() {
            @Override
            public void onLabelCreated(ci.aquarium.net.model.Label label) {
            ...
            @Override
            public void onLabelUpdated(ci.aquarium.net.model.Label label) {
                if (matchesLabelFilter(label.getName())) {
                    fishLabelsCache.put(label.getUid(), label);
                    updateJenkinsLabelsCache();
            ...
            public void onLabelRemoved(String labelUid) {
                ci.aquarium.net.model.Label label = fishLabelsCache.remove(labelUid);
                if (label != null) {
                    updateJenkinsLabelsCache();
    ...
    private void refreshLabels() {
        try {
            List<ci.aquarium.net.model.Label> labels = client.listLabels();
            if (fishLabelsCache == null) {
                this.fishLabelsCache = new ConcurrentHashMap<>();
            }
            fishLabelsCache.clear();
            for (ci.aquarium.net.model.Label label : labels) {
                if (matchesLabelFilter(label.getName())) {
                    fishLabelsCache.put(label.getUid(), label);
                }
            }
    ...
    private void updateJenkinsLabelsCache() {
        ...
        for (ci.aquarium.net.model.Label fishLabel : fishLabelsCache.values()) {
            // Add the base label
    ...
    public Map<String, ci.aquarium.net.model.Label> getFishLabelsCache() {
    ...
    public void startApplicationCreation(AquariumSlave agent, String fishLabel) {
        ...
        ci.aquarium.net.model.Label fishLabelObj = null;
        ...
        for (ci.aquarium.net.model.Label label : fishLabelsCache.values()) {
        ...
        try {
            ...
            ci.aquarium.net.model.Application application = client.createApplication(appBuilder.build());
     ...
     */
    private ci.aquarium.net.model.Label findMatchingFishLabel(Label jenkinsLabel) {
